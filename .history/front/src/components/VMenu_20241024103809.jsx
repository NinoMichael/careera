...
const VMenu = () => {
    ...
    const itemRenderer = (item) => (
        <div className='p-menuitem-content ms-4'>
            <Link className="flex align-items-center p-menuitem-link text-black hover:bg-greenCustom py-2 rounded space">
                <span className={item.icon} />
                <span className="mx-2">{item.label}</span>
                {item.badge && <Badge className="ml-auto font-poppins bg-blackCustom" value={item.badge} />}
            </Link>
        </div>
    )
