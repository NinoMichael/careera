...
const VMenu = () => {
    ...
    const [collapsed, setCollapsed] = useState(false) // Nouvel état pour gérer la réduction du menu

    const toggleMenu = () => {
        setCollapsed(!collapsed) // Alterner entre réduit et étendu
    }
    ...
    const itemRenderer = (item) => (
        <div className={`p-menuitem-content ps-4 hover:bg-greenCustom hover:text-white rounded mb-5 ${collapsed ? 'justify-center' : ''}`}>
            <Link className="flex align-items-center p-menuitem-link text-black py-2">
                ...
                {!collapsed && ( // Cacher le texte quand le menu est réduit
                    <>
    ...
    let items = [
        {
            template: () => {
                return (
                    <div className="my-3 cursor-pointer" onClick={toggleMenu}> {/* Clic pour réduire ou agrandir */}
    ...
    return (
        <div className={`fixed float-left -mt-1 ${collapsed ? 'w-[5vw]' : 'w-[15vw]'}`}> {/* Ajuster la largeur selon l'état */}
