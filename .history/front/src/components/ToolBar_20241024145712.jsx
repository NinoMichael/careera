...
const ToolBar = () => {
    ...
    // Mise à jour de la langue lors du changement
    const handleLanguageChange = (e) => {
        switchLanguage(e.value)  // On met à jour la langue avec le code sélectionné
    }
    ...
    const centerContent = (
        <div className="space-x-24">
            ...
            <div className="flex items-center space-x-3 -mt-7 ms-12">
                ...
                <Dropdown
                    value={langOptions.find(lang => lang.code === language)}  // Assurez-vous que 'language' est le code de langue
                    options={langOptions}
                    onChange={handleLanguageChange}
                    optionLabel="name"  // Afficher les noms (Français, English, etc.)
                    placeholder="Sélectionner la langue"  // Placeholder visible quand aucune langue n'est sélectionnée
                    panelClassName="font-poppins text-xs"
                    className="text-white custom-dropdown-toolbar bg-transparent border-none p-0 outline-none"
                    dropdownIcon={<i className="pi pi-chevron-down text-xs text-white"></i>}
                />
            </div>
        </div>
    )
